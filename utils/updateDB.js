...
const fetchLatestAPIData = async function () {
  ...
  let dayModulator = 6; // Baseline of 6 days. Calculating start date with 6 will calculate the earliest data date to be 6 days from today, meaning today's results will be included in the query.
  ...
  dayModulator +=
    weekday === 6 ? 1 : weekday === 0 ? 2 : weekday === 1 && hour < 12 ? 2 : 0;
  ...
  const startDate = new Date(
    today.getTime() - dayModulator * 24 * 60 * 60 * 1000
  );
...
module.exports = fetchLatestAPIData;
